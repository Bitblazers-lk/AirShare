...
namespace AirShare
{
    public static class FSService
    {
        ...
        public static FSFileAttrib FileType(string e)
        {
            ...
            if (Ext2Type.TryGetValue(ext, out FSFileAttrib attr))
            {
                return attr;
            }
            else
            {
                if (Ext2Mime.TryGetValue(ext, out string mime))
                {
                    if (mime.StartsWith("video"))
                    {
                        return FSFileAttrib.Video;
                    }
                    else if (mime.StartsWith("audio"))
                    {
                        return FSFileAttrib.Audio;
                    }
                    else if (mime.StartsWith("image"))
                    {
                        return FSFileAttrib.Image;
                    }
                    else if (mime.StartsWith("text"))
                    {
                        return FSFileAttrib.Text;
                    }
                }
